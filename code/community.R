...
membership <- data.frame(node_id = as.numeric(V(g)), 
                         comm = as.numeric(V(g)$color))
write.table(membership, "communities.txt", 
            row.names = FALSE, 
            col.names = FALSE, 
            sep = ",")
...
closeness_all <- closeness(g, vids = V(g), 
                           mode = "all",
                           weights = wel$V3, 
                           normalized = TRUE)
...
write.table(closeness, "closeness.txt", 
            row.names = FALSE, 
            col.names = FALSE, 
            sep = ",")
